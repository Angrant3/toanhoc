...
import com.example.bai_cuoi_ki.R.layout.normal_main
...
class normal_main : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val calculationsRef = database.getReference("bai_toan_cap2")
        var dem = 0
        var sobai = 0
        var dung = 0
        var sai = 0
        setContentView(normal_main)
        var dung_final=0
        var sai_final=0
        var sobai_final=0
        ...
        fun handleAnswers(kq: Int) {
            ...
            var kqtmp = kq.toInt()
            val (x, y, z) = makeUniqueRecursive(kq, ran1, ran2)
        ...
        calculationsRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot.exists()) {
                    ...
                    val problemsList = mutableListOf<Pair<String, String>>()

                    for (data in snapshot.children) {
                        val expression = data.child("expression").getValue(String::class.java)
                        val result = data.child("result").getValue(String::class.java)
